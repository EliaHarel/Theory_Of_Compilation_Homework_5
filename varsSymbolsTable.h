        ...
        if (func) {
            ...
            if (type.getType() == Types_enum::SET_TYPE) {
                ...
                CodeBuffer::instance().emit(
                        ptr + " = getelementptr [256 x i1]* , [256 x i1]** %locals_set, i1 0, i1 " + to_string(offset));
...
*/
    ...
    void typeCheckAndAssign(const std::string &id, const Expression &exp) {
        ...
        if (exp.type.getType() != type.getType() && !(type == Types_enum::INT_TYPE && exp.type == Types_enum::BYTE_TYPE)) {
