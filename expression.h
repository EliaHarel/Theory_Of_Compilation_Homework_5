...
public:
    ...
    Expression opOr(const Expression& t){
        ...
        Expression new_var(Types_enum::BOOL_TYPE);
        string phi_comm = new_var.var_name + " = phi i1 [ " + val1 + ", " + false2_label + " ], [ " + val2 + ", " + true_label + " ]";
    ...
    Expression opAnd(const Expression& t){
        ...
        string exit = CodeBuffer::instance().genLabel();
        Expression new_var(Types_enum::BOOL_TYPE);
        string phi_comm = new_var.var_name + " = phi i1 [ " + val1 + ", " + false_label + " ], [ " + val2 + ", " + true2_label + " ]";
        CodeBuffer::instance().emit(phi_comm );
    ...
    static void copySet(const Expression& src, const Expression& dst){

        string init = CodeBuffer::instance().genLabel();
        int to_phi = CodeBuffer::instance().emit("br label @");

        string loop = CodeBuffer::instance().genLabel();
        string cond = gimmeANewCuteVar();
        CodeBuffer::instance().emit( cond + " = icmp eq i32 " + val_in_mem + ", 1");
        int loc = CodeBuffer::instance().emit("br i1 " + cond + ", label @, label @");
        ...
        string phi_comm = counter + " = phi i32 [ 0, " + init + " ], [ " + cnt + ", " + false_label + " ]";
    ...
    Expression in(const Expression& t){
        ...
        CodeBuffer::instance().emit(   val_in_mem + " = load i1 1, i1* " + loc_in_mem);
        ...
        Expression new_var(Types_enum::BOOL_TYPE);
        string phi_comm = new_var.var_name + " = phi i1 [ " + val1 + ", " + false_label + " ], [ " + val2 + ", " + true_label + " ]";
        CodeBuffer::instance().emit(phi_comm );
    ...
    static void checkSetVal(const Expression& set, const Expression& num, const std::string& op){
        ...
        string cond1_code = cond1_name + " = icmp sge i32 " + num.var_name + ", " + to_string(set.type.getSetRange().first);
        ...
        string cond2_code = cond2_name + " = icmp sle i32 " + num.var_name + ", " + to_string(set.type.getSetRange().second);
